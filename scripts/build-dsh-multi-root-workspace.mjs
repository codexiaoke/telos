...
const pluginRoot = resolve(repositoryRoot, 'plugins/dsh-multi-root-workspace')
...
if (clientResult.outputFiles.length !== 1 || clientResult.outputFiles[0]?.text === undefined) {
  throw new Error(`Telos multi-root workspace Client build expected one output, found ${String(clientResult.outputFiles.length)}`)
}
...
if (unexpectedModules.length > 0) throw new Error(`Telos multi-root workspace Client has unsupported dependencies: ${unexpectedModules.join(', ')}`)
...
process.stdout.write('Built Telos DSH multi-root workspace Host and Client plugin.\n')
