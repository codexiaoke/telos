...
private func applicationName(_ bundle: Bundle, fallback: String) -> String {
...
private func installedApplicationURL(bundleId: String?, name: String?) throws -> URL {
    if let bundleId, let url = NSWorkspace.shared.urlForApplication(withBundleIdentifier: bundleId) {
        return url
    }
    if let name, let path = NSWorkspace.shared.fullPath(forApplication: name) {
        return URL(fileURLWithPath: path)
    }
    throw fail("COMPUTER_APP_NOT_FOUND", "no installed application uniquely matches the selector")
}

private func resolveLaunchTarget(_ selector: [String: Any]) throws -> [String: Any] {
    let bundleId = selector["bundleId"] as? String
    let pid = (selector["pid"] as? NSNumber)?.int32Value
    let name = selector["name"] as? String
    if bundleId == nil && pid == nil && name == nil {
    ...
    let runningMatches = runningApps().filter { app in
        if let bundleId, app.bundleIdentifier != bundleId { return false }
        if let pid, app.processIdentifier != pid { return false }
        if let name {
            let display = app.localizedName ?? ""
    ...
    if runningMatches.count > 1 {
        throw fail("COMPUTER_APP_NOT_FOUND", "multiple running processes match the selector; add the exact pid from computer_list_apps")
    }
    if let app = runningMatches.first {
        guard let bundleURL = app.bundleURL else {
            throw fail("COMPUTER_APP_NOT_FOUND", "running application has no launchable bundle URL")
        }
        var target = try appJSON(app)
        target["path"] = bundleURL.path
        return target
    }
    if pid != nil {
        throw fail("COMPUTER_APP_NOT_FOUND", "the selected application process is no longer running")
    }

    let url = try installedApplicationURL(bundleId: bundleId, name: name)
    guard let bundle = Bundle(url: url), let resolvedBundleId = bundle.bundleIdentifier else {
        throw fail("COMPUTER_APP_NOT_FOUND", "the selected application bundle has no identifier")
    }
    if let bundleId, resolvedBundleId != bundleId {
        throw fail("COMPUTER_APP_NOT_FOUND", "the installed application does not match the requested bundleId")
    }
    let resolvedName = applicationName(bundle, fallback: url.deletingPathExtension().lastPathComponent)
    if let name,
       resolvedName.compare(name, options: [.caseInsensitive, .diacriticInsensitive]) != .orderedSame,
       url.deletingPathExtension().lastPathComponent.compare(name, options: [.caseInsensitive, .diacriticInsensitive]) != .orderedSame {
        throw fail("COMPUTER_APP_NOT_FOUND", "the installed application does not match the requested name")
    }
...
private func openApplication(_ target: [String: Any], activateRequested: Bool, timeoutMs: Int) async throws -> [String: Any] {
    ...
    let path = try string(target["path"], "target.path")
    let expectedPid = (target["pid"] as? NSNumber)?.int32Value
    let existing = runningApps().filter { app in
        guard app.bundleIdentifier == bundleId else { return false }
        return expectedPid == nil || app.processIdentifier == expectedPid
    }
    if existing.count > 1 {
        throw fail("COMPUTER_APP_NOT_FOUND", "multiple running processes match the launch target; resolve it again with an exact pid")
    }
    ...
    guard app.bundleIdentifier == bundleId else {
        throw fail("COMPUTER_APP_NOT_FOUND", "macOS launched a different application than requested")
    }
...
private func captureWindow(_ snapshot: ObservationSnapshot, path: String, required: Bool) async throws -> [String: Any]? {
    ...
    if let expectedId {
        // Never silently capture another same-process window after the exact
        // Accessibility window disappeared. A wrong screenshot is more
        // dangerous than a bounded stale-state failure for coordinate input.
        selected = windows.first { $0.windowID == expectedId }
    } else if let expectedTitle, !expectedTitle.isEmpty {
        let titled = windows.filter { $0.title == expectedTitle }
        selected = titled.count == 1
            ? titled.first
            : titled.max { $0.frame.width * $0.frame.height < $1.frame.width * $1.frame.height }
    } else {
        selected = windows.max { $0.frame.width * $0.frame.height < $1.frame.width * $1.frame.height }
    }
    guard let selected else {
        if required && expectedId != nil {
            throw fail("COMPUTER_STALE_OBSERVATION", "the exact observed window is no longer capturable; observe the application again")
        }
...
private func handle(_ request: [String: Any]) async throws -> Any {
    ...
    case "resolve-launch-target":
        return try resolveLaunchTarget(try dictionary(request["selector"], "selector"))
    case "open-app":
        ...
        return try await openApplication(
            try dictionary(request["target"], "target"),
            activateRequested: bool(request["activate"]),
            timeoutMs: actionTimeoutMs
        )
    case "open-settings":
