...
var SMTP_PASSWORD_REF = "TELOS_WORK_REPORT_SMTP_PASSWORD";
...
function plainInline(value) {
  return value.replace(/!\[([^\]]*)]\([^)]*\)/g, "$1").replace(/\[([^\]]+)]\(([^)]+)\)/g, "$1 ($2)").replace(/\*\*([^*]+)\*\*/g, "$1").replace(/__([^_]+)__/g, "$1").replace(/\*([^*]+)\*/g, "$1").replace(/_([^_]+)_/g, "$1").replace(/`([^`]+)`/g, "$1");
}
function markdownToHtmlBody(markdown) {
  ...
  for (const rawLine of lines) {
    ...
    if (heading?.[1] !== void 0 && heading[2] !== void 0) {
      ...
      const sizes = { 1: "24px", 2: "19px", 3: "16px" };
      blocks.push(`<h${String(level)} style="margin:${level === 1 ? "0" : "22px"} 0 12px;font-size:${sizes[level]};line-height:1.35;color:#111827">${inlineHtml(heading[2])}</h${String(level)}>`);
      continue;
    }
    const unordered = /^[-+*]\s+(.+)$/.exec(line);
    if (unordered?.[1] !== void 0) {
      closeParagraph();
      openList("ul");
...
function markdownToPlainText(markdown) {
  return markdown.replace(/\r\n?/g, "\n").split("\n").map((rawLine) => {
    ...
    const heading = /^#{1,6}\s+(.+)$/.exec(line);
    if (heading?.[1] !== void 0) return plainInline(heading[1]);
    const unordered = /^[-+*]\s+(.+)$/.exec(line);
    if (unordered?.[1] !== void 0) return `\u2022 ${plainInline(unordered[1])}`;
    const ordered = /^(\d+)[.)]\s+(.+)$/.exec(line);
    if (ordered?.[1] !== void 0 && ordered[2] !== void 0) return `${ordered[1]}. ${plainInline(ordered[2])}`;
    const quote = /^>\s?(.+)$/.exec(line);
    if (quote?.[1] !== void 0) return plainInline(quote[1]);
    if (/^(-{3,}|_{3,}|\*{3,})$/.test(line.trim())) return "";
    return plainInline(line);
  }).join("\n").replace(/\n{3,}/g, "\n\n").trim();
}
function renderMarkdownEmail(markdown) {
...
function immutableHash(value) {
  return createHash("sha256").update(JSON.stringify(value)).digest("hex");
}
function safeMessage(error, secrets = []) {
  let message2 = error instanceof Error ? error.message : String(error);
  for (const secret of secrets) {
    if (secret !== "") message2 = message2.replaceAll(secret, "[redacted]");
  }
  return message2.replace(/\bsk-[A-Za-z0-9_-]{8,}\b/g, "sk-[redacted]").slice(0, 1e3);
}
function deterministicMessageId(deliveryId, email) {
...
function referenceOf(report) {
  const { markdown: _markdown, ...reference } = report;
  return reference;
}
var WorkReportMailer = class {
  constructor(store, credentials, options) {
    this.store = store;
    this.credentials = credentials;
    this.passwordRef = credentialRef(requiredString(options.passwordRef, "SMTP password credential reference", 128));
    this.createTransport = options.createTransport ?? ((input) => import_nodemailer.default.createTransport(input));
  }
  passwordRef;
  createTransport;
  async prepare(input) {
    const reportId2 = requiredString(input.reportId, "reportId", 64);
    ...
    const additionalMessage = input.additionalMessage === void 0 ? void 0 : requiredString(input.additionalMessage, "additionalMessage", 5e3);
    const [report, directory, mail, credentialInfo] = await Promise.all([
      this.store.get(reportId2),
      this.store.directory(),
      this.store.mailConfig(),
      this.credentials.describe(this.passwordRef)
    ]);
    if (mail === void 0) throw new Error("SMTP settings are not configured");
    ...
    for (const id of groupIds) {
      ...
      if (group === void 0) throw new Error(`recipient group does not exist: ${id}`);
    ...
    for (const id of selectedIds) {
      ...
      if (contact === void 0) throw new Error(`recipient contact does not exist: ${id}`);
    ...
    const source = additionalMessage === void 0 ? report.markdown : `${additionalMessage}
...
${report.markdown}`;
  ...
  async send(input, signal) {
    ...
    if (password === void 0) throw new Error("SMTP password is not configured");
    ...
    try {
      for (const recipient of draft.immutable.recipients) {
        ...
        try {
          ...
          draft.attempts.push({ at, email: recipient.email, status: "sent", ...result2.messageId === void 0 ? {} : { messageId: result2.messageId } });
          await this.store.appendDeliveryHistory({ deliveryId: draft.id, reportId: draft.immutable.report.id, at, email: recipient.email, status: "sent", messageId: result2.messageId ?? null });
        } catch (error) {
          const message2 = safeMessage(error, [password.value]);
          draft.attempts.push({ at, email: recipient.email, status: "failed", error: message2 });
          await this.store.appendDeliveryHistory({ deliveryId: draft.id, reportId: draft.immutable.report.id, at, email: recipient.email, status: "failed", error: message2 });
        }
        await this.store.saveDeliveryDraft(draft);
      }
    } finally {
    ...
    const failed = draft.immutable.recipients.filter((recipient) => !sentEmails.has(recipient.email)).map((recipient) => {
    ...
    if (draft.status === "sent") draft.sentAt = (/* @__PURE__ */ new Date()).toISOString();
    await this.store.saveDeliveryDraft(draft);
    return { deliveryId: draft.id, status: draft.status, sent, failed };
  }
  async checkedDraft(input) {
...
var EMPTY_DIRECTORY = { version: 1, contacts: [], groups: [] };
function errorCode(error) {
  return typeof error === "object" && error !== null && "code" in error && typeof error.code === "string" ? error.code : void 0;
}
function requiredString2(value, field, maxLength = 1e4) {
  if (typeof value !== "string" || value.trim() === "") throw new TypeError(`${field} must be a non-empty string`);
  const normalized = value.trim();
  if (normalized.length > maxLength) throw new RangeError(`${field} is too long`);
  return normalized;
...
function parseDate(value, field) {
  const date = requiredString2(value, field, 10);
...
function reportId(period) {
  return `${period.type}:${period.periodStart}:${period.periodEnd}`;
}
function reportFilename(period) {
  return period.type === "daily" ? `${period.periodStart}.md` : `${period.periodStart}_${period.periodEnd}.md`;
}
function parseReportFilename(type, filename) {
  const match = type === "daily" ? /^(\d{4}-\d{2}-\d{2})\.md$/.exec(filename) : /^(\d{4}-\d{2}-\d{2})_(\d{4}-\d{2}-\d{2})\.md$/.exec(filename);
...
function parseReportId(value) {
  const id = requiredString2(value, "reportId", 64);
  const [type, periodStart, periodEnd, extra] = id.split(":");
  if (extra !== void 0 || type === void 0 || periodStart === void 0 || periodEnd === void 0) {
    throw new TypeError("reportId is invalid");
  }
  return normalizePeriod({ type: parseReportType(type), periodStart, periodEnd });
}
function reportTitle(markdown, period) {
  for (const line of markdown.split("\n")) {
    const match = /^#\s+(.+?)\s*$/.exec(line);
    if (match?.[1] !== void 0) return match[1].trim().slice(0, 300);
  }
  const labels = { daily: "\u5DE5\u4F5C\u65E5\u62A5", weekly: "\u5DE5\u4F5C\u5468\u62A5", monthly: "\u5DE5\u4F5C\u6708\u62A5" };
  return period.periodStart === period.periodEnd ? `${labels[period.type]}\uFF5C${period.periodStart}` : `${labels[period.type]}\uFF5C${period.periodStart} \u81F3 ${period.periodEnd}`;
}
function normalizeMarkdown(value, maxBytes) {
...
function parseContact(value, index) {
  ...
  return {
    ...
    name: requiredString2(input.name, `contacts[${String(index)}].name`, 120),
    email
  };
}
function parseGroup(value, index) {
  ...
  return {
    ...
    name: requiredString2(input.name, `groups[${String(index)}].name`, 120),
...
function parseMailConfig(value) {
  if (typeof value !== "object" || value === null || Array.isArray(value)) throw new TypeError("mail config must be an object");
  const input = value;
  const port = input.port;
  if (!Number.isSafeInteger(port) || port < 1 || port > 65535) throw new RangeError("mail port must be an integer between 1 and 65535");
  if (typeof input.secure !== "boolean") throw new TypeError("mail secure must be a boolean");
  const host = requiredString2(input.host, "mail host", 253);
  if (/\s/.test(host)) throw new TypeError("mail host must not contain whitespace");
  const fromAddress = requiredString2(input.fromAddress, "mail fromAddress", 320).toLowerCase();
  if (!EMAIL_PATTERN.test(fromAddress)) throw new TypeError("mail fromAddress is invalid");
  return {
    host,
    port,
    secure: input.secure,
    username: requiredString2(input.username, "mail username", 320),
    fromName: typeof input.fromName === "string" ? input.fromName.trim().slice(0, 120) : "",
    fromAddress
  };
}
function overlaps(report, start, end) {
  if (start !== void 0 && report.periodEnd < start) return false;
  if (end !== void 0 && report.periodStart > end) return false;
  return true;
}
var WorkReportStore = class {
  rootPath;
  maxReportBytes;
  maxContextBytes;
  ...
  async initialize() {
    await mkdir(this.rootPath, { recursive: true, mode: 448 });
    await chmod(this.rootPath, 448);
    await Promise.all([
      ...REPORT_TYPES.map((type) => mkdir(this.reportDirectory(type), { recursive: true, mode: 448 })),
      mkdir(this.standardDirectory(), { recursive: true, mode: 448 }),
      mkdir(this.deliveryDirectory(), { recursive: true, mode: 448 })
    ]);
  }
  async list(input = {}) {
    await this.initialize();
    const type = input.type === void 0 ? void 0 : parseReportType(input.type);
    const periodStart = input.periodStart === void 0 ? void 0 : parseDate(input.periodStart, "periodStart");
    const periodEnd = input.periodEnd === void 0 ? void 0 : parseDate(input.periodEnd, "periodEnd");
    if (periodStart !== void 0 && periodEnd !== void 0 && periodStart > periodEnd) throw new RangeError("periodStart must not be later than periodEnd");
    const limit = input.limit ?? 100;
    if (!Number.isSafeInteger(limit) || limit < 1 || limit > 500) throw new RangeError("limit must be an integer between 1 and 500");
    const types = type === void 0 ? REPORT_TYPES : [type];
    const reports = [];
  ...
  async context(input) {
    ...
    const sourceType = period.type === "weekly" ? "daily" : period.type === "monthly" ? "weekly" : void 0;
    let sources = [];
    ...
    return {
      ...period,
      standardConfigured: standard !== void 0,
      ...standard === void 0 ? {} : { standard },
      ...existing === void 0 ? {} : { existing },
      ...resolvedSourceType === void 0 ? {} : { sourceType: resolvedSourceType },
      sources: bounded
    };
  }
  ...
  async readStandard(typeValue) {
    const type = parseReportType(typeValue);
    await this.initialize();
    return this.readOptionalText(this.standardPath(type));
  }
  async standards() {
    const entries = await Promise.all(REPORT_TYPES.map(async (type) => [type, await this.readStandard(type) ?? ""]));
    return Object.fromEntries(entries);
  }
  async saveStandard(typeValue, contentValue) {
    const type = parseReportType(typeValue);
  ...
  async directory() {
    ...
    return value === void 0 ? structuredClone(EMPTY_DIRECTORY) : parseDirectory(value);
  }
  async saveDirectory(value) {
  ...
  async mailConfig() {
    ...
    return value === void 0 ? void 0 : parseMailConfig(value);
  }
  async saveMailConfig(value) {
    const config = parseMailConfig(value);
    await this.initialize();
    await this.writeJsonAtomic(this.mailPath(), config);
    return config;
  }
  async saveDeliveryDraft(draft) {
    await this.initialize();
    await this.writeJsonAtomic(this.deliveryPath(assertId(draft.id, "delivery id")), draft);
  }
  async deliveryDraft(idValue) {
    const id = assertId(idValue, "delivery id");
    await this.initialize();
    const value = await this.readOptionalJson(this.deliveryPath(id));
    if (value === void 0) throw new Error("delivery draft does not exist");
    return parseDeliveryDraft(value);
  }
  async appendDeliveryHistory(value) {
    await this.initialize();
    const line = `${JSON.stringify(value)}
`;
    await appendFile(this.historyPath(), line, { encoding: "utf8", mode: 384 });
  }
  async documentsWithin(type, periodStart, periodEnd) {
    const references = (await this.list({ type, periodStart, periodEnd, limit: 500 })).filter((report) => report.periodStart >= periodStart && report.periodEnd <= periodEnd).sort((left, right) => left.periodStart.localeCompare(right.periodStart));
    return Promise.all(references.map((report) => this.get(report.id)));
  }
  async readPeriod(period) {
    await this.initialize();
    const path = this.reportPath(period);
  ...
  async pathExists(path) {
    return await this.safeFileMetadata(path) !== void 0;
  }
...
`);
  ...
  reportDirectory(type) {
    return resolve(this.rootPath, "reports", type);
  }
  reportPath(period) {
    return resolve(this.reportDirectory(period.type), reportFilename(period));
  }
  standardDirectory() {
    return resolve(this.rootPath, "standards");
  }
  standardPath(type) {
    return resolve(this.standardDirectory(), `${type}.md`);
  }
  deliveryDirectory() {
    return resolve(this.rootPath, "delivery-drafts");
  }
  deliveryPath(id) {
    return resolve(this.deliveryDirectory(), `${id}.json`);
  }
  directoryPath() {
    return resolve(this.rootPath, "contacts.json");
  }
  mailPath() {
    return resolve(this.rootPath, "mail.json");
  }
  historyPath() {
    return resolve(this.rootPath, "send-history.jsonl");
  }
};
function documentWithoutBody(report) {
  const { markdown: _markdown, ...reference } = report;
  return reference;
}
function parseDeliveryDraft(value) {
  if (typeof value !== "object" || value === null || Array.isArray(value)) throw new TypeError("delivery draft is invalid");
  const draft = value;
  if (draft.version !== 1 || typeof draft.id !== "string" || typeof draft.createdAt !== "string" || typeof draft.hash !== "string" || !["prepared", "sending", "partial", "sent"].includes(String(draft.status)) || typeof draft.immutable !== "object" || draft.immutable === null || !Array.isArray(draft.sentEmails) || !Array.isArray(draft.attempts)) {
...
// src/index.ts
var name = "telos-work-report";
var inject = ["agents", "connection", "credentials", "tools"];
var JSON_OUTPUT = {
  schema: { type: "json" },
...
function assertTopLevelAgent(ctx, exec) {
  const agent = exec.agent;
  if (agent === void 0 || ctx.agents.get(agent.id) !== agent || !ctx.agents.roots().includes(agent) || ctx.agents.currentInitiator() !== agent || agent.status !== "running") {
    throw new Error("email delivery requires the exact live top-level DSH agent");
  }
  return agent;
}
async function settingsView(store, credentials, passwordRef) {
  const [standards, directory, mail, password] = await Promise.all([
    store.standards(),
    store.directory(),
    store.mailConfig(),
    credentials.describe(credentialRef2(passwordRef))
  ]);
  const mailView = mail === void 0 ? void 0 : {
    ...mail,
    passwordConfigured: password.configured,
    ...password.source === void 0 ? {} : { passwordSource: password.source },
    passwordWritable: password.writable
  };
  return { rootPath: store.rootPath, standards, directory, ...mailView === void 0 ? {} : { mail: mailView } };
}
function installRpc(ctx, store, passwordRef) {
    ...
    (endpoint, payload) => {
      ...
      if (endpoint === "save-mail") {
        return result(async () => {
          ...
          if (password !== void 0 && password !== null && (typeof password !== "string" || password.length === 0)) {
            throw new TypeError("SMTP password must be a non-empty string or null");
          }
          await store.saveMailConfig(config);
          if (typeof password === "string") await ctx.credentials.set(credentialRef2(passwordRef), password);
          else if (password === null) await ctx.credentials.unset(credentialRef2(passwordRef));
          return settingsView(store, ctx.credentials, passwordRef);
        });
      }
      if (endpoint === "list-reports") return result(() => store.list(typeof payload === "object" && payload !== null ? payload : {}));
      return result(() => {
        throw new TypeError(`unknown work-report endpoint: ${endpoint}`);
      });
    },
    { authority: "loopback" }
  );
}
function installTools(ctx, store, mailer) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      return json(await store.list({
        ...args.report_type === void 0 ? {} : { type: args.report_type },
        ...args.period_start === void 0 ? {} : { periodStart: args.period_start },
        ...args.period_end === void 0 ? {} : { periodEnd: args.period_end },
  ...
  ctx.tools.register(defineTool({
    ...
    isConcurrencySafe: () => true,
    async execute(args) {
      return json(await store.get(args.report_id));
    }
  }));
  ctx.tools.register(defineTool({
  ...
  ctx.tools.register(defineTool({
    ...
    isConcurrencySafe: () => true,
    async execute() {
      return json(await store.directory());
    }
  }));
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      const input = {
        ...
        ...args.contact_ids === void 0 ? {} : { contactIds: args.contact_ids },
        ...args.group_ids === void 0 ? {} : { groupIds: args.group_ids },
        ...args.additional_message === void 0 ? {} : { additionalMessage: args.additional_message }
      };
