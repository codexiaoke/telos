...
try {
  ...
  assert(await dialog.getByText('Smoke candidate requires explicit confirmation', { exact: true }).count() === 0,
    'unconfirmed candidate leaked into the effective-memory filter')
  await dialog.getByRole('button', { name: '待确认', exact: true }).click()
