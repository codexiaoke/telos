...
const pluginRoot = resolve(repositoryRoot, 'plugins/dsh-personalization')
...
if (clientResult.outputFiles.length !== 1 || clientResult.outputFiles[0]?.text === undefined) {
  throw new Error(`Telos personalization Client build expected one output, found ${String(clientResult.outputFiles.length)}`)
}
...
if (unexpectedModules.length > 0) throw new Error(`Telos personalization Client has unsupported dependencies: ${unexpectedModules.join(', ')}`)
...
process.stdout.write('Built Telos DSH personalization Host and Client plugin.\n')
