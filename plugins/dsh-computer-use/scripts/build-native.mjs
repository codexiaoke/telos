...
const FIXTURE_BUNDLE_ID = 'io.telos.dsh-computer-use-fixture'
...
async function run(command, commandArgs, timeoutMs = 300000) {
...
async function buildHelper() {
  ...
  await run('codesign', ['--force', '--sign', '-', '--identifier', 'io.telos.dsh-computer-use-helper', '--timestamp=none', HELPER_OUTPUT])
...
`
  ...
  await run('codesign', ['--force', '--deep', '--sign', '-', '--identifier', FIXTURE_BUNDLE_ID, '--timestamp=none', FIXTURE_APP])
  await run('xcrun', [
    'swiftc', '-swift-version', '5', '-O',
    '-target', `${targetArch}-apple-macos14.0`,
    '-framework', 'AppKit',
  ...
  return { path: FIXTURE_APP, bundleId: FIXTURE_BUNDLE_ID, executable: basename(executable), monitor: MONITOR_OUTPUT }
}
