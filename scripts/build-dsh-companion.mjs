...
const pluginRoot = resolve(repositoryRoot, 'plugins/dsh-companion')
...
if (clientResult.outputFiles.length !== 1 || clientResult.outputFiles[0]?.text === undefined) {
  throw new Error(`Telos companion Client build expected one output, found ${String(clientResult.outputFiles.length)}`)
}
...
if (unexpectedModules.length > 0) throw new Error(`Telos companion Client has unsupported dependencies: ${unexpectedModules.join(', ')}`)
...
process.stdout.write('Built Telos DSH companion Settings plugin.\n')
