...
if (clientResult.outputFiles.length !== 1 || clientResult.outputFiles[0]?.text === undefined) {
  throw new Error(`Telos continuity Client build expected one output, found ${String(clientResult.outputFiles.length)}`)
}
...
if (unexpectedModules.length > 0) {
  throw new Error(`Telos continuity Client has unsupported dependencies: ${unexpectedModules.join(', ')}`)
}
if (compiledClient.includes('react.production.min') || compiledClient.includes('react.development.js')) {
  throw new Error('Telos continuity Client contains a private React copy')
}
...
process.stdout.write('Built Telos DSH continuity Host and Client plugin.\n')
